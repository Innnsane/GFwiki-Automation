...
def update_mission_avg():
    ...
    for avg in avg_dict_new:
        ...
        if not exist:
            avg_add = {"file": avg["file"], "path": avg["path"], "name_a": "", "name": "", "name_b": "", "episode": "", "chapter": ""}
            ...
            for story in story_info:
                ...
                for script in scripts:
                    if avg_add["file"] == script:
                        avg_add["episode"] = story["campaign"]

                        # find mission name
                        for mission in mission_info:
                            if story["mission_id"] == mission["id"]:
                                avg_add["name"] = stc_to_text(mission_text, mission["name"])
                                break
                        break
    ...
    for avg in title_dict:
        if len(avg["path"].split("/")) > 1:
            ...
            if avg["path"].split("/")[1] == "fetter":
                avg["chapter"] = avg["path"].split("/")[2]
                fetter_id = avg["file"]

                for fetter_story in fetter_story_info:
                    if fetter_story["id"] == fetter_id:
                        avg["chapter"] = stc_to_text(fetter_text, f"fetter-1000000{fetter_story['fetter_id']}")
                    ...
                    if skin_id == skin["id"]:
                        ...
                        for skin_class in skin_class_info:
                            if skin_class['id'] == skin['class_id']:
                                avg["chapter"] = stc_to_text(skin_class_text, skin_class['name'])

        elif not avg["chapter"] and avg["episode"] and avg["episode"].find("-") == -1:
